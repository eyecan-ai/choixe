...
@click.option(
    "-o", "--output_file", required=True, help="Output single configuration file."
)
@click.option(
    "--check/--nocheck",
    default=True,
    type=bool,
    help="If flag is TRUE compile procedure will stop if placeholders found.",
)
@click.option(
    "--option",
    "options",
    multiple=True,
    nargs=2,
    help="Bind dataset name to underfolder path",
)
def compile(
